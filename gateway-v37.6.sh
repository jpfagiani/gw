...
# Versão 37.6 — UI: sidebar azul, botões com hover, stat-cards border-top, ícones maiores
# ==============================================================================
...
options {
    ...
    forwarders {
        ...
        8.8.8.8;      // Google DNS — fallback para domínios públicos (github.com, etc)
        1.1.1.1;      // Cloudflare DNS — fallback secundário
    };
    forward first;  // Tenta corporativos primeiro; se falharem, usa 8.8.8.8/1.1.1.1
...
view "wan_view" {
    ...
    recursion yes;
    forwarders { 10.14.8.20; 10.1.6.222; 10.14.8.16; 8.8.8.8; 1.1.1.1; };
    forward first;
    ...
    // Zonas por domínio na view WAN também
    zone "sap.sp.gov.br" {
        type forward;
        forwarders { 10.1.6.222; 10.14.8.16; };
        forward only;
    };
    zone "policiapenal.sp.gov.br" {
        type forward;
        forwarders { 10.14.8.20; 10.14.8.16; };
        forward only;
    };
    zone "sap.gov.br" {
        type forward;
...
view "lan_view" {
    ...
    recursion yes;
    // Forwarders padrão — usados para domínios não mapeados abaixo
    ...
    // -------------------------------------------------------------------------
    // Zonas por domínio — forwarder específico para cada DNS corporativo
    // Resolve o problema de domínios que só existem em um dos DNS
    // -------------------------------------------------------------------------

    // DNS 10.1.6.222 — domínios exclusivos (ex: cartório SAP)
    zone "sap.sp.gov.br" {
        type forward;
        forwarders { 10.1.6.222; 10.14.8.16; };
        forward only;
    ...
    // DNS 10.14.8.20 — domínios exclusivos (ex: GPU Polícia Penal)
    zone "policiapenal.sp.gov.br" {
        type forward;
        forwarders { 10.14.8.20; 10.14.8.16; };
        forward only;
...
<!-- Google Fonts removido: rede sem internet -->
<style>
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
html{-webkit-font-smoothing:antialiased}
body{background:#0d1b2e;color:#dde6f0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:1.5rem}
.wrap{width:100%;max-width:380px}
.logo-area{text-align:center;margin-bottom:2rem}
.logo-icon{width:56px;height:56px;margin:0 auto 1rem;background:linear-gradient(135deg,#1a6fdf,#3a8fff);border-radius:14px;display:grid;place-items:center;font-size:1.5rem}
...
.card-head{background:#1c2128;border-bottom:1px solid #2d333b;padding:.75rem 1.25rem;display:flex;align-items:center;gap:.6rem;font-size:.75rem;color:#545d68;font-family:'Consolas','Courier New',monospace}
.dots{display:flex;gap:5px}
.dot{width:8px;height:8px;border-radius:50%}
.d1{background:#ff5a5a}.d2{background:#ffb830}.d3{background:#3fd87a}
...
.alert-success{background:#081828;border-color:#102840;color:#5ab8ff}
...
.eye:hover{color:#3a8fff}
input[type="password"],input[type="text"]{width:100%;background:#1c2128;border:1px solid #2d333b;border-radius:7px;color:#cdd9e5;font-family:'Consolas','Courier New',monospace;font-size:.85rem;padding:.65rem .8rem .65rem 2.3rem;outline:none;transition:border-color .18s,box-shadow .18s}
input:focus{border-color:#3a8fff;box-shadow:0 0 0 3px rgba(58,143,255,.18)}
input::placeholder{color:#545d68}
.btn{width:100%;padding:.75rem;border-radius:8px;background:linear-gradient(90deg,#1a6fdf,#3a8fff);border:1px solid #3a8fff;color:#fff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;font-size:.88rem;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:.5rem;transition:all .18s}
.btn:hover{opacity:.9}
.hint{text-align:center;font-size:.72rem;color:#545d68;line-height:1.55}
code{font-family:'Consolas','Courier New',monospace;color:#5ab8ff;font-size:.8rem}
</style>
</head>
<body>
<div class="wrap">
...
<style>
:root{
  --bg:#0d1b2e;--surf:#112240;--surf2:#163052;--surf3:#1a3a62;
  --border:#1e4070;--border2:#255090;
  --text:#d4e8f8;--text2:#9abcd4;--text3:#5a8ab4;--text4:#3a5e7a;
  --accent:#3a8fff;--accent2:#1a6fdf;
  --green:#3fd87a;--green-bg:#0a2518;--green-bd:#1a4a30;
  --red:#ff5a5a;--red-bg:#2a0f0f;--red-bd:#4a1f1f;
  --yellow:#ffb830;--yellow-bg:#2a1f08;--yellow-bd:#4a3510;
  --cyan:#30d8f0;--cyan-bg:#081e28;--cyan-bd:#103040;
  --purple:#9a7aff;--purple-bg:#160e28;--purple-bd:#2a1e48;
  --blue:#5ab8ff;--blue-bg:#081828;--blue-bd:#102840;
  --orange:#ff9a3a;--orange-bg:#281408;--orange-bd:#3c2010;
  --mono:'Consolas','Courier New',monospace;--sans:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;
...
.brand-icon{width:28px;height:28px;background:linear-gradient(135deg,#1a6fdf,#3a8fff);border-radius:7px;display:grid;place-items:center;font-size:14px;font-weight:700;color:#fff;flex-shrink:0}
...
.np.active{color:var(--accent);background:var(--blue-bg);border-color:var(--blue-bd);font-weight:600}
...
.si.active{color:var(--accent);background:var(--blue-bg);border-color:var(--blue-bd);font-weight:600}
...
.stat-card.c-green{border-top-color:var(--green)}.stat-card.c-green::after{background:var(--green)}
.stat-card.c-blue::after{background:var(--blue)}
.stat-card.c-red{border-top-color:var(--red)}.stat-card.c-red::after{background:var(--red)}
.stat-card.c-yellow{border-top-color:var(--yellow)}.stat-card.c-yellow::after{background:var(--yellow)}
...
.ab{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.4rem;padding:.75rem .5rem;border-radius:var(--rl);font-size:.75rem;font-weight:600;cursor:pointer;border:1px solid;text-align:center;line-height:1.2;min-height:64px;text-decoration:none;transition:all .18s}
.ab-icon{font-size:1.25rem;line-height:1}
.ab.gr{background:var(--green-bg);border-color:var(--green-bd);color:var(--green)}.ab.gr:hover{background:var(--green);color:#0d1b2e;border-color:var(--green)}
.ab.gr:hover{background:var(--green);color:#1c2128;border-color:var(--green)}
.ab.bl{background:var(--blue-bg);border-color:var(--blue-bd);color:var(--blue)}.ab.bl:hover{background:var(--blue);color:#0d1b2e;border-color:var(--blue)}
.ab.bl:hover{background:var(--blue);color:#1c2128;border-color:var(--blue)}
.ab.cy{background:var(--cyan-bg);border-color:var(--cyan-bd);color:var(--cyan)}.ab.cy:hover{background:var(--cyan);color:#0d1b2e;border-color:var(--cyan)}
.ab.cy:hover{background:var(--cyan);color:#1c2128;border-color:var(--cyan)}
.ab.yl{background:var(--yellow-bg);border-color:var(--yellow-bd);color:var(--yellow)}.ab.yl:hover{background:var(--yellow);color:#0d1b2e;border-color:var(--yellow)}
.ab.yl:hover{background:var(--yellow);color:#1c2128;border-color:var(--yellow)}
.ab.pu{background:var(--purple-bg);border-color:var(--purple-bd);color:var(--purple)}.ab.pu:hover{background:var(--purple);color:#fff;border-color:var(--purple)}
.ab.pu:hover{background:var(--purple);color:#1c2128;border-color:var(--purple)}
.ab.rd{background:var(--red-bg);border-color:var(--red-bd);color:var(--red)}.ab.rd:hover{background:var(--red);color:#fff;border-color:var(--red)}
...
.add-form input:focus,.add-form select:focus{border-color:var(--accent);box-shadow:0 0 0 2px rgba(58,143,255,.14)}
...
.btn-add-green:hover{background:var(--accent);color:#fff;border-color:var(--accent)}
...
.accent-blue .card-head{border-left-color:var(--accent)}
...
.accent-blue .dot{background:var(--accent)}
...
textarea:focus{border-color:var(--accent);box-shadow:0 0 0 2px rgba(58,143,255,.12)}
...
<div class="shell">
  ...
  <aside class="sidebar">
    ...
    <a href="#sec-ips" class="si active"><span class="si-ic" style="color:var(--accent)">⊕</span> IPs Totais
    ...
    <a href="#sec-sites" class="si"><span class="si-ic" style="color:var(--accent)">✓</span> Sites Liberados</a>
      ...
      <div class="card" id="sec-sched">
        ...
        <div class="card-body" style="padding:.65rem .8rem">
          ...
          <div class="add-form">
            ...
            <button class="btn-add btn-add-blue" onclick="addSchedule()">
...
<head>
    ...
    <!-- Google Fonts removido: rede sem internet -->
    <!-- Font Awesome CDN removido: rede sem internet -->
    <!-- Chart.js CDN: se sem internet, relatório gráfico não funcionará -->
    <!-- Alternativa: copie chart.umd.min.js para /var/www/gateway-panel/public/ e ajuste o src -->
    <style>
        :root{
            ...
            --accent:#5b7fff;
            --mono:'Consolas','Courier New',monospace; --sans:'Inter',sans-serif;
